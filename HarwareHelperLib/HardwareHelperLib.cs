...
//IF THIS DOESN'T WORK, REBUILD WITH THE CORRECT .NET VERSION.
...
namespace HardwareHelperLib
{
    ...
    public class HH_Lib
    {
        ...
        {
            return SetDeviceState(deviceName, true);
        }

        public bool DisableDevice(string deviceName)
        {
            return SetDeviceState(deviceName, false);
        }

        public bool SetDeviceState(string match, bool bEnable)
        {
            string[] input = { match };
            return SetDeviceState(input, bEnable);
        }
